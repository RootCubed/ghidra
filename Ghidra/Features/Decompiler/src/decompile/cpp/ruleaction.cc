...
/// \class RuleCleanSignBitCompare
/// \brief Simplify the signbit trick that CodeWarrior produces to compare
/// against 0.
/// Without this optimisation, the decompiler would produce something like:
///     INT_RIGHT(INT_2COMP(INT_ZEXT(x)), 31)

/// With this optimisation, the decompiler should produce:
///     INT_NOTEQUAL(var, 0)

void RuleCleanSignBitCompare::getOpList(vector<uint4> &oplist) const

{
  // We want to match on the INT_RIGHT operation
  oplist.push_back(CPUI_INT_RIGHT);
}

int4 RuleCleanSignBitCompare::applyOp(PcodeOp *op, Funcdata &data)

{
  // 1. Check that we matched on "var >> 31" (INT_RIGHT(var, 31))
  // 31 is generalised as the size of the left varnode (in bits) - 1
  ...
  if (left_up->code() != CPUI_INT_2COMP) return 0;

  PcodeOp* left_up_up = left_up->getIn(0)->getDef();
  if (left_up_up->code() != CPUI_INT_ZEXT) return 0;
  Varnode* variable = left_up_up->getIn(0);
  if (variable->isConstant()) return 0;

  // All checks succeeded - now we need to transform this into 'INT_NOTEQUAL(variable, 0)'
  Varnode* zero_constant = data.newConstant(variable->getSize(), 0);

  data.opSetOpcode(op, CPUI_INT_NOTEQUAL);
  data.opSetInput(op, variable, 0);
  data.opSetInput(op, zero_constant, 1);

  data.opDestroy(left_up_up);
  data.opDestroy(left_up);

  return 1;
}

/// \class RuleCleanBoolSubpiece
/// \brief Simplify the signbit trick that CodeWarrior produces to compare
/// against 0.
/// Without this optimisation, the decompiler would produce something like:
///     SUBPIECE41(<bool var>, 0)

/// With this optimisation, the decompiler should produce:
///     <bool var>

void RuleCleanBoolSubpiece::getOpList(vector<uint4> &oplist) const

{
  // We want to match on the SUBPIECE operation
  oplist.push_back(CPUI_SUBPIECE);
}

int4 RuleCleanBoolSubpiece::applyOp(PcodeOp *op, Funcdata &data)

{
  // 1. Check that we matched on SUBPIECE(<bool var>, 0)
  Varnode* left = op->getIn(0);
  Varnode* right = op->getIn(1);

  if (left->isConstant()) return 0;
  if (!right->constantMatch(0)) return 0;

  // 2. Check that var is a boolean var
  if (! left->getDef()->isBoolOutput()) return 0;

  // All checks succeeded - now we need to remove the SUBPIECE
  // We do this by copying the CPUI code and inputs of the <bool var> to the
  // SUBPIECE, and then removing the original PcodeOp
  PcodeOp* bool_op = left->getDef();
  data.opSetOpcode(op, bool_op->code());

  for (int i = 0; i < op->numInput(); i++) {
    data.opSetInput(op, bool_op->getIn(i), i);
  }

  for (int i = op->numInput(); i < bool_op->numInput(); i++) {
    data.opInsertInput(op, bool_op->getIn(i), i);
  }

  // Remove the original PcodeOp
  data.opDestroy(bool_op);
